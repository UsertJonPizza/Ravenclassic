...
CCoinControl* AssetControlDialog::coinControl = new CCoinControl();
...
void AssetControlDialog::updateView()
{
    ...
    for (auto assetPair : mapCoins ) {
        for (const std::pair<QString, std::vector<COutput>> &coins : assetPair.second) {
            ...
            if (treeMode) {
                ...
                // asset name
                itemWalletAddress->setText(COLUMN_ASSET_NAME, assetPair.first);
            }

            std::map<std::string, CAmount> mapSums;
            CAmount nSum = 0;
            int nChildren = 0;
            for (const COutput &out : coins.second) {
                ...
                if (strAssetName != assetPair.first.toStdString())
                    continue;

                if (!mapSums.count(strAssetName))
                    mapSums.insert(std::make_pair(strAssetName, 0));

                mapSums.at(strAssetName) += nAmount;
                nSum += nAmount;
                nChildren++;
